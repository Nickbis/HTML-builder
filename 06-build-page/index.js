...
async function bundleHTML() {
  ...
  for (let i = 0; i < templateTags.length; i++) {
    ...
    const templateHTML = await fs.promises.readFile(
      path.join(__dirname, 'components', `${tagName}.html`), 'utf-8');
...
(async () => {
  try {
    ...
    if (isHTMLBundled && isDirCopy && isStylesBundled) console.log('The files have been created and copied. Task completed.');
  }
})();
